#!/usr/bin/env python3
"""
NVIDIA Canary-1B Flash Gradio Web UI
A comprehensive web interface for the NVIDIA Canary-1B Flash multilingual ASR/AST model.
...
def load_model():
    ...
    try:
        ...
        if canary_model is None:
            ...
            local_model_path = os.path.join(models_dir, "canary-1b-flash")
...
def create_manifest(
    ...
    manifest_data = {
        ...
        "timestamp": "yes" if enable_timestamps else "no"
    }
...
def transcribe_audio(
    ...
    try:
        ...
        if inference_mode == "Short-form":
                        ...
                        if enable_timestamps:
                            try:
                                if hasattr(result, 'timestamp') and result.timestamp:
                                    timestamps_data = result.timestamp
                                    
                                    # Format word-level timestamps
                                    if 'word' in timestamps_data:
                                        word_timestamps = timestamps_data['word']
                                        timestamps_info += "📍 Word-level timestamps:\\n"
                                        for word_info in word_timestamps:
                                            timestamps_info += f"  {word_info}\\n"
                                    
                                    # Format segment-level timestamps  
                                    if 'segment' in timestamps_data:
                                        segment_timestamps = timestamps_data['segment']
                                        timestamps_info += "\\n📍 Segment-level timestamps:\\n"
                                        for segment_info in segment_timestamps:
                                            timestamps_info += f"  {segment_info}\\n"
                            except Exception as ts_error:
                                print(f"Warning: Could not extract timestamps: {ts_error}")
                                timestamps_info = "⚠️ Timestamps requested but could not be extracted"
                    else:
                        transcription = ""
                        timestamps_info = ""
            ...
            try:
                # For long-form, use manifest-based approach
                manifest_path = create_manifest(
                    preprocessed_audio,
                    source_lang_code,
                    target_lang_code,
...
def create_interface():
    ...
    with gr.Blocks(css=css, title="NVIDIA Canary-1B Flash Web UI") as interface:
        
        # Header
        gr.HTML("""
        <div class="header">
            <h1> NVIDIA Canary-1B Flash Web UI</h1>
            <p>Multilingual Automatic Speech Recognition and Translation</p>
            <p><em>Supports English, German, French, and Spanish</em></p>
            <p><em>CVLT AI </em></p>
        </div>
        """)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                <div class="feature-box">
                    ...
                    <ul>
                        <li><strong>Model:</strong> NVIDIA Canary-1B Flash</li>
                        <li><strong>Parameters:</strong> 883M</li>
                        <li><strong>Languages:</strong> English, German, French, Spanish</li>
                        <li><strong>Capabilities:</strong> ASR, AST, Timestamps</li>
...
if __name__ == "__main__":
    print("Starting NVIDIA Canary-1B Flash Gradio Web UI...")
